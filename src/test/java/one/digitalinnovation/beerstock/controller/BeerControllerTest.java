...
public class BeerControllerTest {
    ...
    private static final String BEER_API_SUBPATH_FILTER_URL = "/filter";
    ...
    @Test
    void whenPATCHIsCalledToDecrementGreaterThanStockThenBadRequestStatusIsReturned() throws Exception {

        QuantityDTO quantityDTO = QuantityDTO.builder()
                .quantity(11)
                .build();

        when(beerService.decrement(VALID_BEER_ID, quantityDTO.getQuantity())).thenThrow(EmptyBeerStockException.class);
    ...
    // Filter by critical controller tests

    @Test
    void whenGETListWithCriticalBeersIsCalledThenOkStatusIsReturned() throws Exception {
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();
        beerDTO.setCritical(true);

        Boolean critical = true;
        //when
        when(beerService.findByCritical(critical)).thenReturn(Collections.singletonList(beerDTO));

        // then
        mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH + BEER_API_SUBPATH_FILTER_URL)
                .param("critical", critical.toString())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].name", is(beerDTO.getName())))
                .andExpect(jsonPath("$[0].brand", is(beerDTO.getBrand())))
                .andExpect(jsonPath("$[0].type", is(beerDTO.getType().toString())))
                .andExpect(jsonPath("$[0].critical", is(beerDTO.getCritical())));
    }

    @Test
    void whenGETListWithoutCriticalBeersIsCalledThenOkStatusIsReturned() throws Exception {

        Boolean critical = true;
        //when
        when(beerService.findByCritical(critical)).thenReturn(Collections.EMPTY_LIST);
